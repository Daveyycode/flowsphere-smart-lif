...
const CEODashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'users' | 'revenue' | 'system' | 'insights'
  >('overview')
  ...
  const Modal: React.FC<{ title: string; onClose: () => void; children: React.ReactNode }> = ({
    ...
    >
      <div
        className="rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-auto"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {ceoInsights.map((insight, index) => (
            ...
            >
              ...
              <button
                className="text-sm font-medium flex items-center hover:opacity-70 transition"
            ...
            >
              ...
              <span className="font-bold text-xl" style={{ color: colors.primary }}>
                ...
                {subscriptionBreakdown
                  .reduce((acc, tier) => acc + tier.revenue, 0)
                  .toLocaleString()}
              </span>
            </div>
          </div>
        ...
        <div className="space-y-4">
          {revenueHistory.map((data, index) => (
            <div key={index} className="flex items-center gap-4">
              <div className="w-12 text-sm font-medium" style={{ color: colors.textMuted }}>
                {data.month}
              </div>
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="space-y-4">
          {[
            {
              action: 'Launch email campaign for at-risk users',
              impact: 'Save $1,247 MRR',
              confidence: 87,
            },
            {
              action: 'Extend trial period from 3 to 7 days',
              impact: '+18% conversion rate',
              confidence: 92,
            },
            {
              action: 'Bundle AI features in Gold tier',
              impact: '+$4,200 MRR potential',
              confidence: 78,
            },
            {
              action: 'Implement annual billing discount',
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: colors.background }}>
      {/* Header */}
      <header
        className="border-b sticky top-0 z-40 shadow-sm"
        style={{ backgroundColor: colors.surface, borderColor: colors.border }}
      >
            ...
            <div className="flex items-center space-x-4">
              ...
              >
                <button
                  ...
                  style={{
                    backgroundColor: themeMode === 'light' ? colors.surface : 'transparent',
                  }}
                >
                  <SunIcon
                    className="w-5 h-5"
                    style={{ color: themeMode === 'light' ? colors.warning : colors.textMuted }}
                  />
                </button>
                <button
                ...
                >
                  <MoonIcon
                    className="w-5 h-5"
                    style={{ color: themeMode === 'dark' ? colors.info : colors.textMuted }}
                  />
                </button>
                <button
                ...
                >
                  <ComputerDesktopIcon
                    className="w-5 h-5"
                    style={{ color: themeMode === 'auto' ? colors.primary : colors.textMuted }}
                  />
                </button>
              </div>
            ...
            {[
              {
                ...
                tip: 'Annual billing discounts improve cash flow and reduce churn by 40%',
              },
              {
                title: '📊 Data-Driven Decisions',
                tip: 'Gold tier users engage 3x more with AI features - bundle more capabilities',
              },
              {
                title: '👥 User Retention',
                tip: 'Users enabling 3+ features in week 1 have 85% retention rate',
              },
              {
                title: '🚀 Growth Strategy',
                tip: 'Family plan has 80:1 LTV:CAC ratio - focus marketing here',
              },
            ].map((tip, index) => (
              <div
                key={index}
