      ...
      <header className="sticky top-0 z-40 border-b border-border bg-background/80 backdrop-blur-xl">
        ...
        >
          <div className="flex items-center space-x-2">
            ...
            <span
              className={cn(
                'font-heading font-bold bg-gradient-to-r from-blue-mid to-accent bg-clip-text text-transparent',
        ...
        <>
          ...
          >
            <div className={cn('grid grid-cols-5', bottomNavHeight)}>
              {tabs
                .filter(tab =>
                  ['dashboard', 'notifications', 'devices', 'family', 'settings'].includes(tab.id)
                )
                .map(tab => {
                  const Icon = tab.icon
                  const isActive = currentTab === tab.id
