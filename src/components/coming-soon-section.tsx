...
export function ComingSoonSection({ onNavigate }: ComingSoonSectionProps) {
  ...
  const features: ComingSoonFeature[] = [
    {
      ...
      description:
        'Accessible across devices: perfect for focus sessions or shared family time and pep talks.',
      icon: Timer,
    ...
    },
    {
      ...
      description:
        "User data stored locally on their device (encrypted within the phone OS), not in FlowSphere's cloud.",
      icon: ShieldCheck,
      iconColor: 'text-green-400',
      bgColor: 'bg-green-500/20',
      isAvailable: true,
    ...
    },
    {
      ...
      description:
        'Unified hub: AI Tutor, Study Monitor & Focus Tracking — all in one place under Family tab.',
      icon: Brain,
      iconColor: 'text-green-400',
      bgColor: 'bg-green-500/20',
      isAvailable: true,
    ...
    },
    {
      ...
      description:
        'Connect to Google Home, Alexa & smart devices. Control lights, thermostats & more.',
      icon: Devices,
      iconColor: 'text-green-400',
      bgColor: 'bg-green-500/20',
      isAvailable: true,
    ...
    },
    {
      ...
      description:
        'AI auto-manages your week — syncs with Google Calendar, suggests optimal times & tracks progress.',
      icon: CalendarCheck,
      iconColor: 'text-green-400',
      bgColor: 'bg-green-500/20',
      isAvailable: true,
    ...
    },
    {
      ...
      title: 'Offline Mode',
      description:
        'Essential tools work without internet. Data syncs automatically when back online.',
      icon: WifiSlash,
      iconColor: 'text-green-400',
      bgColor: 'bg-green-500/20',
      isAvailable: true,
  ...
  const handleFeatureClick = (feature: ComingSoonFeature) => {
    if (feature.isAvailable && feature.navigateTo && onNavigate) {
      ...
      toast.success(`${feature.title} is available!`, {
        ...
        <CardContent className={cardPadding}>
          <div className={cn('flex items-center justify-between', isMobile ? 'mb-4' : 'mb-4')}>
            <div className={cn('flex items-center min-w-0', isMobile ? 'space-x-2' : 'space-x-3')}>
              <div
                className={cn(
                  'rounded-full bg-yellow-500/20 flex items-center justify-center flex-shrink-0',
                  isMobile ? 'w-10 h-10' : 'w-12 h-12'
                )}
                style={{ boxShadow: '0 0 20px -5px rgba(234, 179, 8, 0.4)' }}
              >
                <Lock
                  className={cn('text-yellow-500', isMobile ? 'w-5 h-5' : 'w-6 h-6')}
                  weight="fill"
                />
              </div>
              <div className="min-w-0">
                <h2 className={cn('font-bold text-gray-100', isMobile ? 'text-xl' : 'text-2xl')}>
                  Coming Soon...
                </h2>
              ...
              return (
                ...
                >
                  ...
                  {feature.isSoon && !feature.isAvailable && (
                    <div className="absolute top-2 right-2">
                      <span className="px-1.5 py-0.5 text-[9px] font-bold bg-yellow-500 text-yellow-900 rounded">
                        SOON
                      </span>
                    </div>
                  )}
                  <div
                    className={cn('flex items-start justify-between', isMobile ? 'mb-2' : 'mb-3')}
                  >
                    <div
                      className={cn(
                    ...
                    >
                      <IconComponent
                        className={cn(feature.iconColor, isMobile ? 'w-4 h-4' : 'w-5 h-5')}
                        weight="duotone"
                      />
                    </div>
                  </div>
                  ...
                  {feature.isAvailable && (
                    <p
                      className={cn(
                        'mt-1 text-green-400 font-medium',
                  ...
                  {feature.isSoon && !feature.isAvailable && (
                    <p
                      className={cn(
                        'mt-1 text-yellow-400 font-medium',
                        isMobile ? 'text-[9px]' : 'text-[10px]'
                      )}
                    >
                      Coming very soon
