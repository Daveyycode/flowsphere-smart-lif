...
export function DashboardView({
  ...
  const availableBoxes: QuickAccessBox[] = [
    ...
    },
    {
      ...
      description: 'Quick access hotlines',
      icon: Lightning,
      color: 'destructive',
      action: 'emergency',
    },
  ...
  const [quickAccessBoxes, setQuickAccessBoxes] = useKV<string[]>('flowsphere-quick-access', [
    'weather',
    'emergency',
    'meeting-notes',
    'traffic',
  ...
  const selectedBoxes = availableBoxes.filter(box =>
    (
      quickAccessBoxes || ['weather', 'emergency', 'meeting-notes', 'traffic', 'locks', 'family']
    ).includes(box.id)
  )
  ...
  return (
    <div className={cn('space-y-3 pb-4', isMobile && 'space-y-2.5', isTablet && 'space-y-3')}>
        ...
        >
          <div
            className={cn(
              ...
              isMobile ? 'space-y-2.5' : 'space-y-3'
            )}
          >
            <div
              className={cn(
            ...
            >
              <BookOpen
                className={cn('text-white', isMobile ? 'w-4 h-4' : 'w-4.5 h-4.5')}
                weight="fill"
              />
            </div>
            ...
            {!currentVerse ? (
              <>
                <div>
                  <h2
                    className={cn(
                      'font-semibold mb-1.5 font-heading',
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div
                    className={cn('flex flex-wrap justify-center', isMobile ? 'gap-1.5' : 'gap-2')}
                  >
                    {isReading ? (
                      <Button
                        variant="destructive"
                      ...
                      <Button onClick={handleStartReading} size="sm" className="min-touch-target">
                        <Play className="w-4 h-4 mr-2" weight="fill" />
                        Read Aloud
                      </Button>
                    )}
            ...
            >
              ...
              >
                <CardContent className={cn(isMobile ? 'p-2.5' : 'p-3')}>
                  <div
                    className={cn('flex items-start justify-between', isMobile ? 'mb-2' : 'mb-2.5')}
                  >
                    <div
                      className={cn(
                        'rounded-xl flex items-center justify-center',
                        getColorClasses(stat.color).bgLight,
                  ...
                  <div
                    className={cn('flex items-baseline', isMobile ? 'space-x-1' : 'space-x-1.5')}
                  >
        ...
        <Card className="border-blue-mid/20 bg-gradient-to-br from-blue-light/10 via-blue-mid/5 to-transparent">
          <CardHeader className={cn(isMobile ? 'pb-1.5 pt-2.5 px-3' : 'pb-2 pt-3 px-4')}>
            <div className="flex items-center justify-between">
              <CardTitle
                className={cn('flex items-center space-x-1.5', isMobile ? 'text-sm' : 'text-base')}
              >
          ...
          <CardContent
            className={cn('grid pt-0 px-3 pb-3', isMobile ? 'gap-2' : 'gap-2.5', quickAccessCols)}
          >
            {selectedBoxes.map((box, index) => {
              ...
              return (
                ...
                >
                  <Icon
                    className={cn(
                      `${colors.text} mb-1 group-hover:scale-110 transition-transform`,
            ...
            <div className={cn(isMobile ? 'space-y-2' : 'space-y-2.5')}>
              {recentActivity.map((activity, index) => (
                <motion.div
                  ...
                  className={cn(
                    'flex items-start pb-3 border-b border-border/50 last:border-0 last:pb-0',
        ...
        <Card className="border-primary/30 bg-gradient-to-br from-primary/10 via-accent/5 to-coral/10 relative overflow-hidden hover:shadow-lg transition-all">
          ...
          <CardContent className={cn('relative', isMobile ? 'p-3' : 'p-4')}>
            <div className={cn('flex items-start', isMobile ? 'space-x-2.5' : 'space-x-3')}>
              <div
                className={cn(
                  'rounded-full bg-gradient-to-br from-accent to-primary flex items-center justify-center flex-shrink-0',
                  isMobile ? 'w-7 h-7' : 'w-8 h-8'
                )}
              >
                <BookOpen
                  className={cn('text-white', isMobile ? 'w-3.5 h-3.5' : 'w-4 h-4')}
                  weight="fill"
                />
              </div>
              <div className="flex-1 min-w-0">
                ...
                >
                  ...
                  gracious to you." - Numbers 6:24-25
                </p>
                <p
                  className={cn(
                    'text-muted-foreground mb-1.5',
                ...
                >
                  Remember: Your faithfulness and love for others reflects God's light in this
                  world. Every small act of kindness matters.
                </p>
                <button
                  onClick={() => {
                    const scriptures = [
          ...
          <div className={cn(isMobile ? 'space-y-3 pt-3' : 'space-y-4 pt-4')}>
            ...
            <div className="space-y-2">
              {availableBoxes.map(box => {
                const Icon = box.icon
                const isSelected = (
                  quickAccessBoxes || ['emergency', 'meeting-notes', 'traffic', 'locks', 'family']
                ).includes(box.id)
                const colors = getColorClasses(box.color)
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <Icon
                        className={cn(colors.text, isMobile ? 'w-4 h-4' : 'w-5 h-5')}
                        weight="duotone"
                      />
                    </div>
                    ...
                    {isSelected && (
                      <Badge
                        variant="secondary"
                        className={cn('bg-mint/20 text-mint', isMobile ? 'text-[10px]' : 'text-xs')}
                      >
                        Selected
                      </Badge>
                    )}
      ...
      <EmergencyDialog isOpen={showEmergencyDialog} onClose={() => setShowEmergencyDialog(false)} />
    </div>
  )
}
