...
export function Vault({ isOpen, onClose }: VaultProps) {
  ...
  return (
              ...
              <TabsContent value="settings" className="mt-0">
                <Card>
                  ...
                  <CardContent className="space-y-6">
                    ...
                    {typeof window !== 'undefined' && localStorage.getItem('flowsphere_ceo_authenticated') === 'true' && (
                      <div className="pt-8 flex justify-center">
                        <button
                          onClick={() => {
                            setShowCEOTOTP(true)
                          }}
                          className="opacity-10 hover:opacity-30 transition-opacity duration-500"
                          aria-label="CEO Portal Access"
                        >
                          <ShieldCheck className="w-6 h-6 text-muted-foreground" weight="duotone" />
                        </button>
                      </div>
                    )}
                  </CardContent>
    ...
    {/* CEO TOTP Verification Modal */}
    <Dialog open={showCEOTOTP} onOpenChange={setShowCEOTOTP}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="totp">Enter 6-digit TOTP code</Label>
            <Input
              id="totp"
              type="text"
              placeholder="000000"
              ...
              onClick={() => {
                ...
                if (totpCode.length === 6) {
                  toast.success('CEO Dashboard Access Granted', {
                    description: 'Welcome to executive overview',
                    duration: 3000
                  })
                  setShowCEOTOTP(false)
                  setTotpCode('')
