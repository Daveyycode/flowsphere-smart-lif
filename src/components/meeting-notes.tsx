...
export function MeetingNotes() {
  ...
  useEffect(() => {
    ...
    // Use Whisper transcription (more accurate) when:
...
Generated by FlowSphere Meeting Notes`
  ...
  const sendEmail = async () => {
    ...
    if (isResendConfigured() && emailData.note) {
      // Use Resend API for direct email delivery
      try {
        const result = await sendMeetingNotesEmail({
          to: emailTo,
          subject: emailSubject,
          formattedContent: emailData.script,
          noteTitle: emailData.note.title,
          noteDate: emailData.note.date,
          noteDuration: emailData.note.duration,
          formatType: emailData.type.charAt(0).toUpperCase() + emailData.type.slice(1),
          includeRawTranscript: true,
          rawTranscript: emailData.note.transcript,
        })
      ...
      // Fallback to mailto protocol if Resend not configured
